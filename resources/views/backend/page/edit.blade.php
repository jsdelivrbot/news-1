        ...
        <div class="span12">
            <div class="widget-box">
                <div class="widget-title"><span class="icon"> <i class="icon-align-justify"></i> </span>
                    <h5>Sayfa Düzenle : {{$page->tittle}}</h5>
                </div>
                <div class="widget-content nopadding">
                    <form id="pageForme" class="form-horizontal">


                        <div class="control-group">
                            <label class="control-label">Sayfa Başlığı</label>
                            <div class="controls">
                                <input type="text" name="tittle" value="{{$page->tittle}}" class="span11"/>
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">Yazı İçeriği</label>
                            <div class="controls">
                                <textarea name="content" id="editor" cols="30" class="span11" rows="10">{!! $page->content !!}</textarea>
                        ...
                        <div class="form-actions text-right">
                            <button type="button" id="pageUpdate" class="btn btn-success">Güncelle</button>
                        </div>
                    </form>
                </div>
            </div>
...
@push("customJs")
    ...
    <script>
        ...
        $("#pageUpdate").on("click", function () {
            var form = new FormData($("#pageForme")[0]);
            ...
            $.ajax({
                type: "post",
                url: "{{route("backend.page.update",["id"=>$page->id])}}",
                data: form,
                processData: false,
                contentType: false,
